...
Conduct a boxploting to find out outliers in $DF_1$ and $DF_2$:
...
Find and plot highly correlated variables ($r$>0.6 in $df_1$:


```python
corr = df1.corr()
high_corr = corr[np.abs(corr) > 0.6].fillna(0)
...
Potential reduntant variables found, however, now the data is prepared for analyzing.
...
Though data have been prepared in section *Data Preparation*, dataset $df_1$ has been imported again here following problem description.
...
```python
# Df1 trainset
df1_train = df1[df1['Year']<=2006]
# Check the result
df1_train.iloc[[1, 2,-2, -1],:]
```
...
```python
# Df1 testet
df1_test = df1[df1['Year']>2006]
# Check the result
df1_test.iloc[[1, 2,-2, -1],:]
```
...
*Implement a function `closed_form_1` that computes this closed form solution given the features $𝐗$, labels $Y$ (using Python or Matlab).*
...
def closed_form_1(X:np.ndarray, y:np.ndarray) -> np.ndarray:
    """
...
def closed_form_2(df: pd.core.frame.DataFrame, column:int=10) -> np.ndarray:
    """
    ...
    ----------
        ...
        column = 10: index number of clomn where DV y lies. The default value is 10.
    ...
    X = np.concatenate([np.ones((len(X),1)), X], axis = 1) # add x0 = 1 to matrix X
    y = df.iloc[:, [column]].to_numpy()                    # y: the results, lower case to emphasize the difference
    theta = inv(X.T @ X) @ X.T @ y
    #theta = theta[1:].reshape((1,10))
    return theta
```

Test `closed_form_1` and `closed_form_2` on $df_1$:


```python
# Given X, and y
X = df1_train.drop(df1_train.columns[10], axis=1).to_numpy() # X: the features
y = df1_train.iloc[:, [10]].to_numpy()                       # y: the results, lower case to emphasize the difference
...
```python
# Given a DataFrame
theta = closed_form_2(df1_train).reshape((1,11))
theta
```
    ...
    array([[-1.42447514e+02,  8.23964853e-03, -3.61230814e-03,
             6.44651665e-02,  2.51139099e-03,  1.87173441e-04,
            -1.63078493e-02, -6.27428717e-03,  3.42652472e-03,
             9.51816161e-02, -1.54295993e+00]])
...
$R^2$ measures model fitting and can be calculated as:
...
```python
def score(y: np.ndarray, y_hat:np.ndarray):
    """
    ...
    Return:
    ----------
        rsqure: R^2 caculated based on y and y_hat.
    """
    
    mean = y.mean()
    nomi = np.sum(np.square(y_hat - mean))
    deno = np.sum(np.square(y - mean))
    rsquare = nomi / deno
    return rsquare
```
...
```python
rsquare = score(y, predict(X, closed_form_1(X, y)))
print("R2:", rsquare)
```

    R2: 0.7549422939270283
...
```python
X = df1_train.drop(df1_train.columns[10], axis=1).to_numpy()
l.score(X, y)
```
...
Based on the formula above, $R^2$ can be applied in Python to evaluate previous model:
...
parameter λ (using Python or Matlab).3. Compare the two solutions in problem 1 and problem 2 and explain the reason why linear model with L2 regularization is robust. (using climate_change_1.csv)*

### 4. Change λ
*You can change the regularization parameter λ to get different solutions for this problem. Suppose we set λ = 10, 1, 0.1, 0.01, 0.001, and please evaluate the model*  $R^2$ *on the training set and the testing set. Finally, please decide the best regularization parameter λ. (Note that: As a qualified data analyst, you must know how to choose model parameters, please learn about cross validation methods.)*
